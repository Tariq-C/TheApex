...
public class FightAndFlight extends AbstractDynamicCard {
    ...
    public static final String ID = TheApexMod.makeID(FightAndFlight.class.getSimpleName()); // USE THIS ONE FOR THE TEMPLATE;
    ...
    private static final int NEXT_ENERGY = -1;

    // /STAT DECLARATION/


    public FightAndFlight() { // - This one and the one right under the imports are the most important ones, don't forget them
        super(ID, IMG, COST, TYPE, COLOR, RARITY, TARGET);
        baseMagicNumber = magicNumber = ENERGY;
        defaultBaseSecondMagicNumber = defaultSecondMagicNumber = NEXT_ENERGY;
        this.exhaust = true;
    }
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        AbstractDungeon.actionManager.addToBottom(
                new ApplyPowerAction(p, p, new EnergizedPower(p,defaultSecondMagicNumber))
        );
    }
